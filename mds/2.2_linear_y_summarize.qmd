...
tb_results_knockoff <- 
  tb_job_knockoff$i_job %>% 
  purrr::map_dfr(function(i_job) {
    # in rare occasions (3/800 here), knockoff run can fail, which we omit
    if(!file.exists(paste0(dir_output, "/fit/knockoff_", i_job, ".RData")))
    ...
    load(paste0(dir_output, "/data/y_sim_", i_job, ".RData"))
    load(paste0(dir_output, "/fit/knockoff_", i_job, ".RData"))
    pos <- fit_knockoff[, 1] == 0
    tibble::tibble(
      power = mean(pos[y_sim$ind_TP]),
      fdr = {
        if(!any(pos))
      ...
      method = "Fixed-X knockoff",
      i_job = i_job)
  })
tb_results_knockoff <- tb_results_knockoff %>% 
  dplyr::left_join(tb_job, by = "i_job")
save(tb_results_knockoff, file = paste0(dir_output, "/tb_results_knockoff.RData"))
...
colors <- c("black", rev(palette.colors(n = 6, "R4")[-c(1, 3)]))
names(colors) <- c("DA", "Fixed-X knockoff", "LinDA", 
                   "lasso (no inference)", "CompDA")
...
p_effect_power <- tb_results_summary %>% 
  dplyr::filter(method != "Fixed-X knockoff") %>% 
