...
filename = [pwd,'\IBD data analysis\Stability.mat'];
save(filename, 'result');

filename = [pwd,'\IBD data analysis\sample.mat'];
...
filename3 = [pwd,'\IBD data analysis\Stability.csv'];
csvwrite(filename3, stability);


%stability of not using constraints
penalize =0;
result = cell(nrep);
parpool('local',2);
parfor i=1:nrep
    ...
    for j=1: length(Lambda)
        lambda = Lambda(j);
        [betahat,betahat_nset0]=byapg(penalize,'p',y,x2, 0, lambda, 1, 50000, 1e-6);
        supp = betahat==0;
        supp = supp(2:78);
        Supp(j,:) = supp;
    end
...
filename = [pwd,'\IBD data analysis\Stability_noc.mat'];
save(filename, 'result');
...
filename3 = [pwd,'\IBD data analysis\Stability_noc.csv'];
csvwrite(filename3, stability);
